...
\begin{enumerate}
    ...
    \begin{enumerate}[(a)]
        ...
        By the factorization, we know that $n (\hat{\beta} - \beta) \sim \exp(\lambda)$, therefore $\sqrt{n} (\hat{\beta} - \beta) \cp 0$.
        Recall the CLT we have that $\sqrt{n}\left\{\bar{X} - (\frac{1}{\lambda} + \beta)\right\} \cp N(0, \frac{1}{\lambda^2})$, by Slutsky's Theorem we have:
        $$\sqrt{n}(\bar{X} - X_{(1)} - \frac{1}{\lambda}) = \sqrt{n}(\bar{X} - \frac{1}{\lambda} - \beta) - \sqrt{n}(X_{(1)} - \beta) \cp N(0, \frac{1}{\lambda^2})$$
        By delta method we have:
        $$\sqrt{n}(\hat{\lambda} - \lambda) \cp N(0, \lambda^2)$$
